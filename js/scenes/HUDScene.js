class HUDScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.shockwaveBar = this.add.rectangle(700, 50, 100, 10, 0x00ffff);
        this.shockwaveText = this.add.text(650, 45, 'SHOCK:', { fontSize: '16px', fill: '#fff' });
        
        this.teleportBar = this.add.rectangle(700, 70, 100, 10, 0x00ffff);
        this.teleportText = this.add.text(650, 65, 'TELE:', { fontSize: '16px', fill: '#fff' });

        // Add powerup indicator
        this.powerupText = this.add.text(16, 96, '', {
            fontSize: '24px',
            fill: '#fff'
        });

        // Listen for powerup changes
        this.scene.get('GameScene').events.on('powerupChanged', (powerupType) => {
            if (powerupType) {
                this.powerupText.setText(`POWERUP: ${powerupType.toUpperCase()}`);
            } else {
        ...
        gameScene.events.on('shockwaveUpdate', this.updateShockwaveBar, this);
        gameScene.events.on('teleportUpdate', this.updateTeleportBar, this);
    }
    ...
    updateShockwaveBar(percent) {
        this.shockwaveBar.width = percent;
        this.shockwaveBar.fillColor = percent === 100 ? 0x00ffff : 0x666666;
    }

    updateTeleportBar(percent) {
        this.teleportBar.width = percent;
        this.teleportBar.fillColor = percent === 100 ? 0x00ffff : 0x666666;
    }
} 